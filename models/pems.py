...
class DSTAGNN_block(nn.Module):
    ...
    def forward(self, x, m, res_att):
        ...
        # miss graph learning
        # adj_pa = get_k_nn_neighbor(x_TAt, k=10 * self.num_of_vertices)
        ...
        # mask = m[:, :, :1, :].expand(batch_size, num_of_vertices,
        #                              num_of_features,
        #                              num_of_timesteps).permute(0, 2, 1,
        #                                                        3)  # B,F,N,T
        # X_mask = X * mask
        x_gtu = []
        ...
        # time_conv = time_conv * (torch.sum(torch.ones_like(mask)) /
        #                          torch.sum(mask))
...
class DSTAGNN_submodule(nn.Module):
    ...
    def __init__(self, DEVICE, num_of_d, nb_block, in_channels, K,
        ...
        self.fc_hid = nn.Sequential(
            nn.Conv2d(nb_time_filter, 128, (1, 1), (1, 1)), nn.Linear(12, 1))
        self.DEVICE = DEVICE
